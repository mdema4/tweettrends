...
def optimize_vocabulary():
    ...
    vocabulary_before = marshal.load(vocabulary_file)
    ...
    for i, word in enumerate(vocabulary_before):
        word_likelihood_dict[word] = [word_likelihood[0][i], word_likelihood[1][i]]

    for key, value in word_likelihood_dict.iteritems():
        print key, value


    word_likelihood_file = open('word_likelihood_dict.dat', 'wb')
    marshal.dump(word_likelihood_dict, word_likelihood_file)
    word_likelihood_file.close()
